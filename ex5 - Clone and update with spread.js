...
function updateStudentGrade(studentsArray, idToUpdate, newGrade) {
  ...
  let indexToUpdate = -1; // Changeable varible should not declare using 'const' keyword
  indexToUpdate = studentsArray.findIndex(student => student.id === idToUpdate) ? 0 : -1;
  ...
  if (indexToUpdate !== -1) {
    ...
    studentsCopy[idToUpdate - 1].grade = newGrade; 
    return studentsCopy;
  } else {
